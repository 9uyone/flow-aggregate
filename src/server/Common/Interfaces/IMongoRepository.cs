...
public interface IMongoRepository<T> where T : class {
	...
	Task<(List<T> items, int totalCount)> FindAsync(FilterDefinition<T> filter, SortDefinition<T>? sort = null, int? page = 1, int? pageSize = 10);
	Task<List<T>> FindAllAsync(FilterDefinition<T> filter, SortDefinition<T>? sort = null);
